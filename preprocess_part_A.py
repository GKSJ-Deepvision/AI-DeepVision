"""
Preprocessing for ShanghaiTech Part A - Crowd Counting Dataset
Includes:
...
def load_part_a_data():
    """Load ShanghaiTech Part A dataset"""
    root_dir = r"E:\DeepVision\ShanghaiTech\part_A"
    ...
    print("="*70)
    print("[PREPROCESSING] ShanghaiTech PART A")
    print("="*70)
    ...
    output_dir = r"E:\DeepVision\processed_dataset\part_A"
    ...
    with open(os.path.join(output_dir, 'part_A_dataset.pkl'), 'wb') as f:
        pickle.dump(data_dict, f)
    
    # Save as NPZ
    np.savez_compressed(
        os.path.join(output_dir, 'part_A_dataset.npz'),
        X_train=X_train_augmented,
        X_test=X_test,
        y_density_train=y_density_train_augmented,
        y_density_test=y_density_test,
    ...
    print(f" [OK] Saved: {os.path.join(output_dir, 'part_A_dataset.pkl')}")
    print(f" [OK] Saved: {os.path.join(output_dir, 'part_A_dataset.npz')}")
    ...
    print("\n [DONE] PART A PREPROCESSING COMPLETE!")
    return data_dict

if __name__ == "__main__":
    data = load_part_a_data()
